...
if __name__ == "__main__":
    ...
    # for i in range(1,6):
    #     for j in range(1, 6):
    #         feature["q_quote"].append(f"rank_{i}_{j}")
    for j in range(1, 6):
        feature[f"rank_{1}_{j}"] = [f"rank_{1}_{j}"]
    ...
    # Get unique id then shuffle it
    student_id = np.unique(df['user_id'].values)
    np.random.seed((random_state))
    np.random.shuffle(student_id)

    # Split into difffernt set
    train_id = student_id[:140]
    valid_id = student_id[140:170]
    testing_id = student_id[170:]

    # Prepare data for training - use a simple train/test split for now

    keys = feature.keys()

    comb = [list(combinations(keys, i)) for i in range(1, len(keys) + 1)]
    result = {}
    df1 = df[new_names + ["label", "user_id"]]
    for groups in comb:
        for words in groups:
            t = []
            for f in list(words):
            ...
            y_train = []
            for id in train_id:
                for array in df[df['user_id'] == id].values:
                    x_train.append(array)
                    y_train.append(array[-2])
            for id in valid_id:
                for array in df[df['user_id'] == id].values:
