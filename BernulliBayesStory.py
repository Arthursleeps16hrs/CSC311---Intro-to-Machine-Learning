...
def bootstrap(data_size: int, boot_size: int, data, label):
    index_array = np.random.choice(data_size, boot_size)
    boot_data = []
    boot_label = []
...
def bootstrap_train(X_train, t_train, num_iteration, boot_ratio):
    pi_list = []
    theta_list = []
    boot_size = int(X_train.shape[0] * boot_ratio)

    for _ in range(num_iteration):
        X, t = bootstrap(X_train.shape[0], boot_size, X_train, t_train)
        pi, theta = train_model(X, t)
        pi_list.append(pi)
        theta_list.append(theta)

    final_pi = np.mean(np.array(pi_list), axis=0)
    final_theta = np.mean(np.array(theta_list), axis=0)

    return final_pi, final_theta
...
def train_model(x_train, y_train):
    ...
    pi_map, theta_map = naive_bayes_map(x_train, y_train)
...
def predict(num_iteration, boot_ratio, kernal: int, x, y):
    ...
    **Graded Task**: Complete the function `make_prediction` which uses our estimated parameters $\pi$ and $\theta_{jc}$ to make predictions on our dataset.
    ...
    for i in [[2, 3]]:
        ...
        x1_train, y1_train = x_train, y1_train.values

        pi_map, theta_map = bootstrap_train(x1_train, y1_train, num_iteration, boot_ratio)
        print(theta_map)
...
if __name__ == "__main__":
    ...
    predict(num_iteration=500, boot_ratio=0.2, kernal=1, x=x, y=y)
